...
| `MCP_URL_KEY` | No | Optional shared secret appended to the public HTTP endpoint URL |
...
- When `MCP_URL_KEY` is set, every HTTP request must use `?key=<token>` in the MCP URL.

## Protecting A Public HTTP Deployment

If you deploy the MCP server to Vercel or Railway, set `MCP_URL_KEY` to a long random secret.
...
```text
https://YOUR-APP.vercel.app/mcp?key=YOUR_SECRET
```

Quick verification:

```bash
curl -i -X POST 'https://YOUR-APP.vercel.app/mcp?key=YOUR_SECRET' \
...
Without the correct URL key, the server returns `401 Unauthorized`.
